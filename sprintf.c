...
int main() {
  ...
  sprintf(s1, "%+5.12f %% %-2.0f %-40.20d % 12f %-20c %-40.15fA\n", 0.0,
          -123.456, 1982634, 34531345.34565342, 'f',
...
void int_function(char **str) {
  ...
  int nulls =
      f.accuracy
          ? ((f.accuracy - num_length(d)) < 0 ? 0 : f.accuracy - num_length(d))
          : 0;
...
void double_function(char **str) {
  ...
  int spaces = f.length - num_length(dub) - f.acc_flag -
               (f.accuracy ? f.accuracy : 6) - f.plus - f.space -
               (dub < 0 ? 1 : 0);
...
void string_function(char **str) {
  ...
  f.accuracy =
      f.accuracy > (int)strlen(str_copy) ? (int)strlen(str_copy) : f.accuracy;
